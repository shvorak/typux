...
var ClassInfo = (function (_super) {
    ...
    ClassInfo.prototype.ensureProperty = function (name) {
        ...
        var length = this.properties.push(new PropertyInfo(name));
        return this.properties[length - 1];
    };
    ClassInfo.prototype.ensureMethod = function (name) {
        var exists = this.methods.find(function (x) { return x.name === name; });
        if (exists) {
            return exists;
        }
        var length = this.methods.push(new MethodInfo(name, this.type.prototype[name]));
